 ...
 * behavior.
 */
contract Crowdsale is Pausable, ReentrancyGuard {
  using SafeMath for uint256;
  ...
  constructor(uint256 rate, uint256 minimumAmount, address wallet, IERC20 token) public {
    require(rate > 0);
  ...
  function buyTokens(address beneficiary) public nonReentrant payable {
    ...
    //HC : mapping 해놓기
    _ethBalances[beneficiary] = _ethBalances[beneficiary].add(msg.value);
  ...
  function withdraw(address user) public onlyOwner{
    _wallet.transfer(_balances[user]);
  }

  // /**
  //  * @dev refund ETH      qeustion : refund는 이더리움을 돌려주는거라 민감한 코드인데 수동으로 하는게 좋지 않을까 생각된다.
  //  */
  // function refund(address user) public onlyOwner{
  //   _ethBalances[beneficiary];
  // }
